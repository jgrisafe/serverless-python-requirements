 ...
 * @param {array} include sibling modules to include in all functions
    ...
    .then(sourceZip =>
      sourceZip.filter(
        file =>
          ...
          include.find(inc => file.startsWith(inc + '/')) ||
...
function injectAllRequirements(funcArtifact) {
  ...
  if (this.serverless.service.package.individually) {
    return BbPromise.resolve(this.targetFuncs)
      ...
      .map(func => {
        if (func.module !== '.') {
          const artifact = func.package && func.package.artifact ? func.package.artifact : funcArtifact;

          const newArtifact = path.join('.serverless', `${func.name}.zip`);

          func.package.artifact = newArtifact; // test
